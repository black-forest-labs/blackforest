...
from bfl.types.base.output_format import OutputFormat


class ImageInput(BaseModel):
    """Batch image-source descriptor (path, folder, zip, or base64 data).

    A legacy helper retained for back-compat; most callers pass images directly
    to ``generate``/``submit`` via ``images=[...]`` instead.
    """
...
class GenericImageValidation:
    ...
    def _validate_image(cls, image: str, field_name: str = "image"):
        ...
        # Pillow is an optional dependency; only the image-validating tool
        # models (fill/expand) need it. Import lazily so that
        # `import bfl` works without Pillow installed.
        try:
            import io
            ...
            raise ValueError(
                "Pillow is required to validate image inputs for this model. "
                "Install it with `pip install 'black-forest-labs[images]'`."
            ) from exc
...
class GenericImageInput(BaseModel):
    ...
    prompt: Optional[str] = Field(
        default="",
        json_schema_extra={"example": "ein fantastisches bild"},
    ...
    output_format: Optional[OutputFormat] = Field(
        ...
        description="Output format for the generated image: 'jpeg', 'png', or 'webp'.",
    )
    ...
    webhook_url: Optional[str] = Field(
        default=None,
        description="URL to receive a webhook POST when the task completes.",
    )
    webhook_secret: Optional[str] = Field(
        default=None,
        description="Secret echoed back verbatim in the X-Webhook-Secret header "
        "so you can verify the callback came from BFL.",
    )
